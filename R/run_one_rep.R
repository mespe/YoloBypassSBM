...
#'
#' Run one replicate of the model; a replicate is a water_year/chinook_run/sim_type combination
#'
#' @md
...
run_one_rep <- function(water_year, chinook_run = c("Fall", "LateFall", "Spring", "Winter"),
  ...
  entrain_list <- entrainment(model_days, knights_abun, sim_type)
  ...
  if (nrow(yolo) > 0){
    ...
    yolo[["YoloRearingTime"]] <- rearing_time_yolo(yolo[["KnightsDay"]], sim_type)
